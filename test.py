...
def percept():
    ...
    for itr in range(30):
        ...
        for doc in records:
            ...
            if y*activation <= 0:
                ...
                cache_posnegA = cache_posnegA + (y * x_vector)
                Bias[0] += y
                cache_b[0] += (y * counter)
            else:
                cache_posnegA += cache_posnegA
            ...
            if y*activation <= 0:
                ...
                cache_TFA = cache_TFA + (y * x_vector)
                Bias[1] += y
                cache_b[1] += (y * counter)
            else:
                cache_TFA += cache_TFA
    ...
    percepmodel_write(Weight_posnegV.tolist(), Weight_TFV.tolist(), Bias.tolist(), cache_posnegA.tolist(), cache_TFA.tolist(), Bias_avg.tolist(), unique_words)
